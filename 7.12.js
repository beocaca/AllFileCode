...
function removeUser(id, e) {
    ...
    xmlhttp.open("DELETE", "https://5fb3f39db6601200168f817f.mockapi.io/users/" + id)
    xmlhttp.setRequestHeader("Content-Type", "application/json");
    xmlhttp.send(null);
    console.log(id)
}

function editName(id, e) {
    var xmlhttp = new XMLHttpRequest();
    newName = window.validateName(prompt('Enter your name :'))
    if (!newName){
        return
    }
    xmlhttp.onreadystatechange = function () {
    ...
    xmlhttp.open("PUT", "https://5fb3f39db6601200168f817f.mockapi.io/users/" + id)
    xmlhttp.setRequestHeader("Content-Type", "application/json");
    xmlhttp.send(JSON.stringify({name : newName}));
    console.log(id)
}
...
function newUser() {
    ...
    xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 201) {
            newObj = (JSON.parse(this.response))
            addUser(newObj)
            //console.log(this);
            console.log(xmlhttp , this.response)
        }
    };
    xmlhttp.open("POST", "https://5fb3f39db6601200168f817f.mockapi.io/users/")
    xmlhttp.setRequestHeader("Content-Type", "application/json");
    xmlhttp.send(JSON.stringify({name: newName}));
}
...
window.onload = function () {
    ...
    xmlhttp.open("GET", "https://5fb3f39db6601200168f817f.mockapi.io/users/")
    xmlhttp.setRequestHeader("Content-Type", "application/json");
    xmlhttp.send();
}
